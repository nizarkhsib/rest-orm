...
export class DataReader<T> {
    ...
    protected _httpAuthBackendService: BackendService;
    ...
    getAll(): Observable<T[]> {
        return new Observable<T[]>(observer => {
            ServiceLocator.injector.get(BackendService).get(this.ressourceUrl, this.options)
                .subscribe(
                    (res: T[]) => {
                        observer.next(
    ...
    getOne(id: number): Observable<T> {
        return new Observable<T>(observer => {
            this._httpAuthBackendService.getOne(this.ressourceUrl + '/' + id, this.options)
                .subscribe(
                    (res: T) => {
    ...
    getBySpecificId(key: string, value: number): Observable<T[]> {
        return new Observable<T[]>(observer => {
            this._httpAuthBackendService.get(this.ressourceUrl + '/' + key + '/' + value, this.options)
                .subscribe(
                    (res: T[]) => {
                        observer.next(
                            res.map(e => this.assignToInstance<T>(JSON.stringify(e), new this.ctr()))
